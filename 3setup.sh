...
# ノードの役割を入力 (1: router, 2: data_host, 3: interest_sender)
read -p "このノードの役割を入力してください (1: router, 2: data_host, 3: interest_sender): " NODE_ROLE

# フォワーディング戦略を入力 (1: best-route, 2: multicast, 3: ncc, 4: best-route-2)
read -p "このノードに適用するフォワーディング戦略を入力してください (1: best-route, 2: multicast, 3: ncc, 4: best-route-2): " STRATEGY_CHOICE

# 戦略のパスを設定
case $STRATEGY_CHOICE in
  1) STRATEGY_PATH="/localhost/nfd/strategy/best-route" ;;
  2) STRATEGY_PATH="/localhost/nfd/strategy/multicast" ;;
  3) STRATEGY_PATH="/localhost/nfd/strategy/ncc" ;;
  4) STRATEGY_PATH="/localhost/nfd/strategy/best-route-2" ;;
  *)
    echo "無効なフォワーディング戦略が選択されました。" >&2
    exit 1
    ;;
esac
...
# ノードのIPアドレスと役割、およびコストを入力
NODE_INFO=()
for (( i=1; i<=NODE_COUNT; i++ )); do
  read -p "接続されているノード $i のIPアドレスを入力してください: " NODE_IP
  read -p "ノード $i の役割を入力してください (1: interest_sender, 2: data_host, 3: router): " NODE_TYPE_CHOICE
  read -p "ノード $i へのコストを入力してください: " NODE_COST
  case $NODE_TYPE_CHOICE in
    1) NODE_TYPE="interest_sender" ;;
    2) NODE_TYPE="data_host" ;;
    3) NODE_TYPE="router" ;;
    *)
      echo "無効なノードの役割が選択されました。" >&2
      exit 1
      ;;
  esac
  NODE_INFO+=("$NODE_IP,$NODE_TYPE,$NODE_COST")
done
...
# ノードごとにFaceの作成と確認、コストの設定
for NODE in "${NODE_INFO[@]}"; do
  IFS=',' read -r NODE_IP NODE_TYPE NODE_COST <<< "$NODE"
  ...
  # コストの設定
  FACE_ID=$(nfdc face list | grep "udp://$NODE_IP" | awk '{print $3}')
  if nfdc face update $FACE_ID cost $NODE_COST; then
    echo "ノード $NODE_IP に対する Face のコストを $NODE_COST に設定しました。"
  else
    echo "ノード $NODE_IP に対する Face のコスト設定に失敗しました。" >&2
    exit 1
  fi
done
...
case $NODE_ROLE in
  1) NODE_ROLE="router" ;;
  2) NODE_ROLE="data_host" ;;
  3) NODE_ROLE="interest_sender" ;;
  *)
